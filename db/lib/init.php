...
$os = stristr(PHP_OS, 'linux') !== false ? '.nix' : '';

if (file_exists(MPM_PATH . '/config/db_config_'.strtolower(APPLICATION_ENV).$os.'.php'))
{
	/**
	 * Include the database connection info.
	 */
	require_once(MPM_PATH . '/config/db_config_'.strtolower(APPLICATION_ENV).$os.'.php');
}
